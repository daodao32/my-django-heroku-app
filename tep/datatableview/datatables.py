...
def columns_for_model(model, fields=None, exclude=None, labels=None, tooltips=None, processors=None,
    ...
    for f in sorted(opts.fields):
        ...
        if tooltips and f.name in tooltips:
        ...
        column = column_class(sources=[f.name], label=label, tooltip = tooltip, processor=processor, sortable=sortable,
                              visible=visible)
        column.name = f.name
...
class DatatableOptions(object):
    ...
    def __init__(self, options=None):
        ...
        self.tooltips = getattr(options, 'tooltips', None)
...
class DatatableMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        if opts.model:
            columns = columns_for_model(opts.model, opts.columns, opts.exclude, opts.labels, opts.tooltips,
            ...
            for name, column in declared_columns.items():
                ...
                try:
                    column.tooltip = opts.tooltips[str(name)]
                except:
                    column.tooltip = None
